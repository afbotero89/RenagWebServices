...
class Asthmapp_test_model extends Asthmapp_model{
	...
	public function getAllByGroup($group_id){
		try{
			$stmt = $this->db->prepare("SELECT * FROM " . $this->tables['test'] . " WHERE group_id = $group_id AND status = 1 AND api_version = " . $this->api_version);
			$stmt->execute();
	...
	public function getAllById($id){
		try{
			$stmt = $this->db->prepare("SELECT * FROM " . $this->tables['test'] . " WHERE id = $id AND status = 1 AND api_version = " . $this->api_version);
			$stmt->execute();
	...
	public function insertOrUpdate($data){
		try{
			if(isset($data['id'])){ //update record
				...
				//Create SQL query
				$sql = "INSERT INTO " . $this->tables['test'] . " (name, document, email, result, date_test, device_id, remote_id, group_id, test_status, api_version) VALUES (?,?,?,?,?,?,?,?,?, " . $this->api_version . ")";
