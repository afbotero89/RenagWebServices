...
class Asthmapp_group_model extends Asthmapp_model{
	...
	public function insertOrUpdate($data){
		try{
			if(isset($data['id'])){ // Update
				...
				if($affected > 0){
					$data_result_update = array(
						"success" => $affected
						);
					$this->response->setResponse(true);
					$this->response->result = $data_result_update;
				}else{
	...
	public function close($id){
		try{
			...
			if($affected > 0){
				$data_result_close = array(
					"success" => $affected
					);
				$this->response->setResponse(true);
				$this->response->result = $data_result_close;
			}else{
				$this->response->setResponse(false, "Error closing group.");
			}
		}catch(Exception $e){
			$this->response->setResponse(false, $e->getMessage());
		}
	...
	public function delete($id){
		try{
			...
			if($affected > 0){
				$data_result_delete = array(
					"success" => $affected
					);
				$this->response->setResponse(true);
				$this->response->result = $data_result_delete;
			}else{
